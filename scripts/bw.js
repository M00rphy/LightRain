window.addEventListener("load", function () {
  ...
  function save_Now() {
    ...
    } else if (this.p.toggled && this.p.label == "Save") {
      this.p.label = "Saving";
      console.log("Saving");
    } else if (!this.p.toggled && this.p.label == "Saving") {
      this.p.label = "Save";
      console.log("Not Saving");
    }
  }
  ...
  */
  ...
  Q.Sprite.extend("Player", {
    init: function (p) {
      this._super(p, { // this overloads the base constructor (init (p, defaults))
        ...
        standingPoints: [
          ...
          [-25, -22],
          [-16, -40],
          [-13, -48], //Top  left point
          //Right Points
          [13, -48], //Top right point
          [16, -40],
          [25, -22],
          [26, 10],
          [20, 9],
    ...
    enemyHit: function (data) {
      ...
      this.p.strength -= enemy.p.damage_p; //Damage caused byt the enemy on collision
    ...
    spikeHit: function (data) {
      ...
      this.p.immuneTimer = 0;
      this.p.immuneOpacity = 1;
      this.p.strength -= spike.p.damage;
      Q.stageScene("hud", 1, this.p);
      this.healthEvaluation();
    },
    ...
    attack: function () {
      //TODO: Create the attacking method
      //TODO: Create the collision mask for attacking
    },

    fallingCalc: function () {
      //TODO: Find a way to check if the player is not landed or falling to evaluate
    ...
    step: function (dt) {
      ...
      if (!processed) {
        ...
        //console.log("Landed thing" + this.p.landed);
  ...
  Q.Sprite.extend("Enemy", {
    ...
    die: function (col) {
      if (col.obj.isA("Player")) {
        ...
        col.obj.p.score += this.p.v_enemy; //Enemy score value
        Q.stageScene("hud", 1, col.obj.p); //Update the hud
        this.play("dead");
        this.p.dead = true;
        var that = this;
        col.obj.p.vy = -300;
  ...
  Q.Sprite.extend("Heart", {
    ...
    sensor: function (colObj) {
      ...
      if (this.p.amount && colObj.p.strength < 100) {
        var strengthMissing = colObj.p.strength_max - colObj.p.strength;
        if (strengthMissing >= this.p.amount) {
          colObj.p.strength += this.p.amount;
        } else if (strengthMissing < this.p.amount) {
          colObj.p.strength += strengthMissing;
        }
        Q.stageScene("hud", 1, colObj.p);
  ...
  Q.scene("Paused_screen", function (stage) {
    var pauseButton = Q("pause_button");
    if (pauseButton != null) {
      console.log("The value of continueFunc = " + pauseButton);
    }
    ...
    }, pauseButton.trigger("toggled")));
    // FIXME: Unpause on click on the continue button;

    var Save = container.insert(new Q.UI.Button({
      label: "Save",
      y: 100,
      x: 0,
      fill: "#CCCCCC",
    ...
    //TODO: Finish implementing the save function
    ...
    //TODO: Make another screen to ask the player if he really wants to exit
    //TODO: If he clicks on yes then clear all and return to the opSelector screen
  ...
  Q.scene("hud", function (stage) {
    ...
    var pause_button = container.insert(new Q.UI.Button({
      label: "Pause",
      y: Q.height / 13.5,
      x: (Q.width / 8.5) * 7.3,
      fill: "#990000",
  ...
  Q.loadTMX(
    ...
    function () {
      ...
      Q.compileSheets("collectables.png", "heart.json");
      Q.animations("player", {
        walk_right: {
...
// TODO: Add support for WASD movement
...
//REMEMBER: https://gamedev.stackexchange.com/questions/76093/saving-game-data-to-server#comment281281_76099
//REMEMBER: https://www.digitalocean.com/community/tutorials/una-introduccion-a-oauth-2-es
//REMEMBER: http://www.html5quintus.com/guide/input.md#.XPlyhnt7lPY
//REMEMBER: http://www.html5quintus.com/guide/scenes.md#.XPl2xXt7lPY
//REMEMBER: http://www.html5quintus.com/guide/core.md#.XPl2yXt7lPY
