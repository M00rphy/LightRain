    ...
    Q.InputSystem = Q.Evented.extend({
        ...
        touchControls: function (opts) {
            ...
            Q.input.keypad = opts = Q._extend({
                left: -35, //NOTE: Determines how far the buttons are from the right edge of the screen.
                gutter: 5, //NOTE: Change the gutter and change the size(?) of the buttons
                controls: DEFAULT_TOUCH_CONTROLS,
                width: Q.width,
                bottom: Q.height,
                fullHeight: false
        ...
        joypadControls: function (opts) {
            ...
            var joypad = Q.joypad = Q._defaults(opts || {}, {
                ...
                trigger: 20,
                center: 25,
                color: "#460606", //NOTE: joypad color
                background: "#eaeaea",
                alpha: 0.5,
                zone: Q.width / 2,
                joypadTouch: null,
        ...
        /**
         * //NOTE: Draw the touch buttons on the screen
         *
        ...
        drawButtons: function () {
            ...
            for (var i = 0; i < keypad.controls.length; i++) {
                ...
                if (control[0]) {
                    ctx.font = "bold " + (keypad.size / 2) + "px Future";
                    var x = keypad.left + i * keypad.unit + keypad.gutter,
                        y = keypad.bottom - keypad.unit,
                        key = Q.inputs[control[0]];

                    ctx.fillStyle = keypad.color || "#460606"; //NOTE: button colors
                    ctx.globalAlpha = key ? 0.75 : 0.35; //NOTE: button opacity, first value: when pressed, second: on idle
                    ctx.fillRect(x, y, keypad.size, keypad.size);

                    ctx.fillStyle = keypad.text || "#ffffff"; //NOTE: button text colors
